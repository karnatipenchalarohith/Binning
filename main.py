...
[open(bin[i-1]+'.txt', 'w').write(spt[i]) for i in range(1, len(bin))]
...
for i, l in enumerate(binned_all_parameters):
...
'''
wb2 = xlwt.Workbook()
wp = wb2.add_sheet('Sheet 1')

for i, l in enumerate(all_splits):
    for j, col in enumerate(l):
        wp.write(i, j, col)

wb2.save( 'binnes2.xls')
f.close()
'''
